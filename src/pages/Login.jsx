...
const Login = () => {
    ...
    return (
        <form onSubmit={onSubmitHandle} className="flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4 text-gray-800">
            ...
            {currentState === 'Login' ? '' : 
            <input type="text" required className="w-full px-3 py-2 border border-gray-800" placeholder="Name"/>
            ...
            <div className="flex w-full justify-between text-sm mt-[-8px]">
                ...
                <p onClick={()=> setCurrentState(()=> {
                        if(currentState==="Login") return "Sign Up"
                        else return "Login"
                    })} className="cursor-pointer">{currentState==="Login" ? "Create Account" : "Login Here"}</p>
